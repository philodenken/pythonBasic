...
def is_between_100_and_999(user_input_number):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    user_input_integer = int(user_input_number)
    if(user_input_integer >= 100 and user_input_integer < 1000):
        result = True
    else:
        result = False
    # ==================================
...
def get_not_duplicated_three_digit_number():
    ...
    while (1):
        tmp_random = str(get_random_number())
        if(not is_duplicated_number(tmp_random)):
            break
...
def get_strikes_or_ball(user_input_number, random_number):
    ...
    for num in user_input_number:
        # check strike
        if(num == random_number[user_input_number.index(num)]):
            result[0]+=1
        else:
            # check ball
...
def is_yes(one_more_input):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당
    input_upper = one_more_input.upper()

    if(input_upper=="Y" or input_upper=="YES"):
        result = True
    else:
        result = False
...
def is_no(one_more_input):
    ...
    if(input_upper=="N" or input_upper=="NO"):
        result = True
    else:
        result = False
    # ==================================
...
def main():
    ...
    while(user_input):
        ...
        if(is_validated_number(user_input)):
            tmp_list = get_strikes_or_ball(user_input,random_number)
            print("Strikes : %d , Balls : %d"%(tmp_list[0], tmp_list[1]))

            if(tmp_list[0]==3):
                one_more_input=999
                while(one_more_input):
