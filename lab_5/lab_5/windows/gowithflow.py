...
def comparison_list_size(list_data_a, list_data_b):
    ...
    # 한 줄 이상의 코드로 작성 가능하나,
    # 반드시 결과 값을 result 변수에 할당하여 반환
    lenth_a = len(list_data_a)
    lenth_b = len(list_data_b)
...
def number_of_cases(list_data):
    ...
    tmp_arr = []
    combine = ""
    for element1 in list_data:
        for element2 in list_data:
            combine = element1 + element2
            tmp_arr.append(combine)
            #combine = ''

    # 중복 제거
    tmp_arr2=[]
    for i in range(len(tmp_arr)):
            if(tmp_arr2.count(tmp_arr[i])==0):
                tmp_arr2.append(tmp_arr[i])
