...
def is_help_command(user_input):
    ...
    user_str = user_input.upper()

    if(user_str == "H" or user_str == "HELP"):
        result = True
    else:
        result = False
...
def is_validated_english_sentence(user_input):
    ...
    # 특수 문자가 있는 경우
    for x in special_char1:
        if (x in user_input):
            result = False

    # (.,!?) 제외하면 입력값이 없거나 빈칸일 경우
    tmp_char =""

    for c in user_input:
        if (not c in special_char2):
            tmp_char += c

    if(len(tmp_char.strip()) == 0):
        result = False
...
def is_validated_morse_code(user_input):
    ...
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    special_char = ['-', '.', ' ']
    morse_code_value_list = get_morse_code_dict().values()
    result = True
    ...
    for word in user_input:
        if(not word in special_char):
            result =False

    #case2
    for word in user_list:
        if (not word in morse_code_value_list):
            result =False
...
def decoding_sentence(morse_sentence):
    ...
    tmp_sentence = morse_sentence.strip()
    tmp_sentence += " "

    for c in tmp_sentence:
        if(c==" "):
            if(len(tmp_char)==0):
                result += " "
            else:
                result+=decoding_character(tmp_char)
                tmp_char=""
        else:
            tmp_char += c
